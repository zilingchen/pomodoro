...
#list
  body(:style='mybgb')
    b-container
        b-col(cols='4')
          b-form-group(label='新增事項' invalid-feedback='請至少輸入兩個字' :state='state' class="text-secondary" )
            b-form-input(v-model='newitem' :state='state' trim @keydown.enter='additem')
            b-btn(variant='secondary' @click='additem') 新增
        br
        b-row
          b-col(cols='6')
            h4(class="text-secondary") 待辦事項
          ...
          b-col(cols='6')
            h4(class="text-secondary") 已完成事項
            b-table-simple
              thead
                tr
                  th(class="text-secondary") 名稱
                  th(class="text-secondary") 操作
              tbody
